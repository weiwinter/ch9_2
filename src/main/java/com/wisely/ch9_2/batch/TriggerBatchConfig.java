...
public class TriggerBatchConfig {
	...
	/*
	 * @Bean public ItemReader<Person> reader(){ FlatFileItemReader<Person> reader =
	 * new FlatFileItemReader<Person>(); reader.setResource(new
	 * ClassPathResource("people.csv")); reader.setLineMapper(new
	 * DefaultLineMapper<Person>() {{ setLineTokenizer(new DelimitedLineTokenizer()
	...
	@Bean
	public JobRepository cvsJobepository(DataSource dataSource, PlatformTransactionManager transactionManager)
			throws Exception {
	...
	public SimpleJobLauncher csvJobLauncher(DataSource dataSource, PlatformTransactionManager transactionManager)
		...
		jobLauncher.setJobRepository(cvsJobepository(dataSource, transactionManager));
		return jobLauncher;
	}
	...
	@Bean
	public Step step1(StepBuilderFactory stepBuilderFactory ,ItemReader<Person> redader,
